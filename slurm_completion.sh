...
function _myjobs() {
echo $( scontrol -o show jobs | grep $USER | cut -d' ' -f 1 | cut -d'=' -f 2 ) ;
}
function _jobs() {
...
function _myjobnames() {
echo $( scontrol -o show jobs | grep $USER | cut -d' ' -f 2 | cut -d'=' -f 2 ) ;
}
function _jobnames() {
...
_scancel()
{
    ...
    --account|-A) offer_list "$(_accounts)" ;;
    --clusters|-M) offer_list "$(_clusters)" ;;
    --name|--jobname|-n) offer_list "$(_myjobnames)" ;;
    --nodelist|-w) offer_list "$(_nodes)" ;;
    --partition|-p) offer_list "$(_partitions)" ;;
    --qos) offer_list "$(_qos)" ;;
    --reservation|-R) offer_list "$(_reservations)" ;;
...
_sbatch()
{
    ...
    --wait-all-nodes) offer_list "1 0" ;;
    --wckey) offer_list "$(_wckeys)" ;;
    :|afterany|after|afternotok|afterok) offer_list "$(_myjobs)" ;;
    *)  _filedir
    esac
}
